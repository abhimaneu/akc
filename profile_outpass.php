...
//for table
$sql = "select * from outpass_old,outpass_products_old where outpass_old.no_year = outpass_products_old.no_year AND outpass_old.user_id = '" . (string) $loggedin_session . "' AND outpass_products_old.user_id = '" . (string) $loggedin_session . "'";
        ...
        <div class="container-fluid mt-1 mb-2 p-2 bg-white rounded-5 shadow-5-strong p-4">
            <table class="table table-sm">
                ...
                <tbody id='tablebody'>
                    ...
                    <td>
                    <a href='createpdfpass.php?f=old&no={$row['no_year']}&io=outpass' target='_blank'>Download</a>
                    </td>
                    ";
                                $cur_no = $row['no'];
                            }
...
if (isset($_POST['filter'])) {
    ...
    echo "<script type='text/javascript'>
            window.location.href = 'profile_outpass.php?f=1&start=$start_date&end=$end_date';
            </script>";
}
...
if (isset($_POST['delete_data'])) {
    ...
    $sql = "DELETE from outpass_old where date BETWEEN '$year_start_date' AND '$year_end_date' AND user_id ='" . (string) $loggedin_session . "'";
    ...
    $sql = "DELETE from outpass_products_old where date_of_entry BETWEEN '$year_start_date' AND '$year_end_date' AND user_id ='" . (string) $loggedin_session . "'";
    $delete = mysqli_query($conn, $sql);
    if (!$delete) {
        echo mysqli_error($conn);
        echo "<script type='text/javascript'>
        window.location.href = 'profile_outpass.php?e=e2';
        </script>";
        exit;
    }

    echo "<script type='text/javascript'>
        window.location.href = 'profile_outpass.php?e=s';
        </script>";
}
?>